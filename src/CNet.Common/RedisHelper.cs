...
namespace CNet.Common
{
    ...
    public class RedisHelper
    {
        ...
        private readonly string redisServer=RedisConnectionHelper.RedisConnectionString.Split(':')[0];
        private readonly string redisPort=RedisConnectionHelper.RedisConnectionString.Split(',')[0].Split(':')[1];
        #region 构造函数
        /// <summary>
        /// </summary>
        ...
        /// <param name="dataKey"></param>
        /// <returns></returns>
        public RedisValue[] HashGet(string key, RedisValue[] dataKey)
        {
            key = AddSysCustomKey(key);
            return Do(db =>
            {
        ...
        public string[] HashKeys(string key)
        {
            ...
            return Do(db =>
            {
                 string[] fields= db.HashKeys(key).ToStringArray();
                 return fields;
            });
        }
        ...
        /// <param name="key"></param>
        /// <param name="value"></param>
        public long ListRightPushBatch(string key, RedisValue[] values, CommandFlags flags = CommandFlags.None)
        {
            key = AddSysCustomKey(key);
            return Do(db => db.ListRightPush(key,values,flags));
        }
        ...
        /// <param name="key"></param>
        /// <param name="value"></param>
        public void ListLeftPushBatch(string key, RedisValue[] value, CommandFlags flags = CommandFlags.None)
        {
            key = AddSysCustomKey(key);
            Do(db => db.ListLeftPush(key,value,flags));
        }
        ...
        /// <param name="value"></param>
        /// <param name="score"></param>
        public long SortedSetAddBatch(string key, SortedSetEntry[] value, CommandFlags flags = CommandFlags.None)
        {
            key = AddSysCustomKey(key);
            return Do(redis => redis.SortedSetAdd(key,value,flags));
        }
        ...
        public IList<RedisKey> GetAllKeys() 
        {
            return GetServer(redisServer,Convert.ToInt32(redisPort)).Keys(DbNum).ToList();
        }
        #endregion key
        ...
        {
            return _conn.GetServer(host,port);
        }
