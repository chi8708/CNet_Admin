...
import { getPage, remove } from "@/api/test_table";
import { test_table } from "@/access/test_table"
export default {
  name: 'test_table',//与 router.js notCache:fasle且name相同 将缓存组件。
  components: {
    Edit,
  },
  data() {
    ...
    return {
      userAccess:{
        isAdd: userAccessAll.includes(`${test_table.ADD}`),
        isEdit: userAccessAll.includes(`${test_table.EDIT}`),
        isMove: userAccessAll.includes(`${test_table.EDIT}`),
      },
      tableData1: [],
      queryData: {},
      pageTotal: 0,
