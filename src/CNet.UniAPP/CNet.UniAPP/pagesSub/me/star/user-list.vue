<template>
	<view class="content">
		<view style="margin-top: 10px;">
			<u-swipe-action v-for="(item,index) in items" :key="item.login">
							...
							<u-row>
							  <u-col  span="9"> 
								<view class="swipe-action__content__text">
										<i class="iconfont icon-kaifazhezhongxin-copy">{{item.login}}</i>
								</view>
								</u-col>
...
<script>
	...
	export default {
		...
		methods: {
			...
			gotoDetail(item){
				uni.navigateTo({
					url:`../../developer/detail?login=${item.login}`
				})
			},
			//取消
			removeStar(item){
			   this.items=this.items.filter(p=>p.login!=item.login)
